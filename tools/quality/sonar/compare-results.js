...
const oldIssues = JSON.parse(
  fs.readFileSync(path.join(DATA, 'sonar-api-issues.json'), 'utf8')
);
const newIssues = JSON.parse(
  fs.readFileSync(path.join(DATA, 'sonar-api-issues-new.json'), 'utf8')
);
...
oldStats.forEach((i) => {
  oldBySev[i.severity] = (oldBySev[i.severity] || 0) + 1;
});
newStats.forEach((i) => {
  newBySev[i.severity] = (newBySev[i.severity] || 0) + 1;
});

console.log('\nOld severity:');
Object.entries(oldBySev)
  .sort((a, b) => b[1] - a[1])
  .forEach(([sev, count]) => {

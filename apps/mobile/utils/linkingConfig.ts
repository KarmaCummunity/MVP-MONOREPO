...
export const linking: LinkingOptions<RootStackParamList> = {
  ...
  getStateFromPath: (path: string, options?: Parameters<typeof defaultGetStateFromPath>[1]) => {
    ...
    if (categoryMatch) {
      ...
      // Use default getStateFromPath with the modified path and full config
      return defaultGetStateFromPath(pathWithMode, { ...options, ...linkingConfig } as Parameters<typeof defaultGetStateFromPath>[1]);
    }
    // Use default behavior for other paths with full config
    return defaultGetStateFromPath(path, { ...options, ...linkingConfig } as Parameters<typeof defaultGetStateFromPath>[1]);
  },

...
import {
    loginWithEmailAndEstablishSession,
    establishSessionFromGoogleResponse,
    establishSessionFromAppleResponse,
} from '../session/loginFlows';
                            ...
                            {Platform.OS === 'web' ? (
                                ...
                                <>
                                    <SecureGoogleAuthButton
                                        ...
                                        onSuccess={async ({ user: u, tokens }) => {
                                            await establishSessionFromGoogleResponse(
                                                {
                                                    success: true,
                                                    user: {
                                                        id: u.id,
                                    ...
                                    <AppleAuthButton
                                        onSuccess={async (serverResponse) => {
                                            await establishSessionFromAppleResponse(serverResponse);
                                            await useUserStore.getState().checkAuthStatus();
                                            await navigationQueue.reset(0, [{ name: 'HomeStack' }], 2);
                                        }}
                                        onError={(msg) => {

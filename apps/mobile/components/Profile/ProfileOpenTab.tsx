/**
 * Profile "Open" tab: active/available posts, items, rides, tasks, donations.
 */
...
import type { ApiPost, CharacterType, ProfileFeedItem } from './types';
import { formatRideTime, getLocationName, safeStr, safeNum } from './profileUtils';
...
export default function ProfileOpenTab({ userId, user, onHeightChange }: Props) {
      ...
      try {
            ...
            res.data.forEach((p: ApiPost) => {
              ...
              if (shouldInclude) {
                ...
                allContent.push({
                  ...
                  taskData: p.task ? { id: String(p.task.id ?? ''), title: String(p.task.title ?? ''), status: String(p.task.status ?? '') } : undefined,
        ...
        try {
          ...
          userRides.forEach((ride: Record<string, unknown>) => {
            ...
            allContent.push({
              ...
              type: 'post',
              subtype: 'ride',
              status: safeStr(ride.status) || 'active',
              from: fromLocation,
              to: toLocation,
        ...
        try {
          ...
          if (donationsRes.success && Array.isArray(donationsRes.data)) {
            const activeDonations = donationsRes.data.filter((donation: Record<string, unknown>) => donation.status === 'active');
  ...
  if (loading) {
    return (
      <View style={styles.tabContentPlaceholder}>
        <Text style={styles.placeholderText}>{t('profile:openContent.loading')}</Text>
      </View>
    );
  }
  ...
  if (posts.length === 0) {
    return (
      <View style={[styles.tabContentPlaceholder, { height: 400 }]} onLayout={(e) => onHeightChange?.(Math.max(400, e.nativeEvent.layout.height))}>
        ...
        <Text style={styles.placeholderText}>{t('profile:openContent.empty')}</Text>
        <Text style={styles.placeholderSubtext}>{t('profile:openContent.emptySubtext')}</Text>
      </View>
    );
  }

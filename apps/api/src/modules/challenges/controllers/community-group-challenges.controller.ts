...
export class CommunityGroupChallengesController {
  ...
  /** Sanitize user-controlled string for safe logging (prevents log injection). */
  private sanitizeForLog(value: string, maxLength = 80): string {
    if (value == null || typeof value !== "string") return "[non-string]";
    return value
      .replace(/[\r\n\t]/g, " ")
      .trim()
      .slice(0, maxLength);
  ...
  async createChallenge(@Body() dto: CreateCommunityGroupChallengeDto) {
    this.logger.log(
      `Creating new community challenge: ${this.sanitizeForLog(dto.title ?? "")}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `🔍 Creating challenge: ${this.sanitizeForLog(dto.title ?? "")}`,
      );
      this.logger.log(
        `📊 Challenge data: creator_id=${dto.creator_id}, type=${dto.type}, frequency=${dto.frequency}, difficulty=${dto.difficulty}`,
      );
  ...
  async joinChallenge(
    ...
    this.logger.log(
      `User ${this.sanitizeForLog(dto.user_id ?? "", 36)} joining challenge ${this.sanitizeForLog(challengeId, 36)}`,
    );

...
| Area | Gap | Evidence |
|------|-----|----------|
| **Identity-resolution proof** | §2.1.6 currently permits `/api/users/resolve-id` to return JWT tokens after lookup by email, Firebase UID, or Google ID, but does not require proof that the caller controls that identifier. This conflicts with §3.2 authentication guarantees and enables account takeover. **Required:** verify a Firebase/Google credential or password before linking identifiers or issuing tokens; email knowledge alone is never proof of identity. | `users.controller.ts` `resolveUserId`; `user-auth.service.ts` `resolveUserId`, `linkResolveIdentifiers`, `buildResolveSuccessResponse` |
...
| **Donations tab category grid vs §2.3.1** | Mobile `DonationsScreen` / `DonationsStack` surface **three** tab entry points (items, rides/trump, challenges) only; money, time, knowledge, food, and housing category screens are not registered. SRS/API may still list additional donation categories — **reconcile** when product scope for the donations tab is finalized. | `bottomBarScreens/DonationsScreen.tsx`; `navigations/DonationsStack.tsx` |

...
| **Chat guest-access contract** | Functional §2.7 documents `OptionalAuthGuard` and guest reads, but security/privacy §3.2 and §3.5 require authorization and server-side data minimization. The current guest path exposes private conversation metadata and messages and accepts a caller-supplied sender identity. **Required:** make chat reads and writes authenticated and participant-scoped, or explicitly define a separate public-chat resource that contains no private conversation data. | `chat.controller.ts` guest branches in conversation/message handlers; §2.7.1; §3.2; §3.5 |

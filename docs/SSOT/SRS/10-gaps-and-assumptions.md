...
| **`resolve-id` identity-proof contract is incomplete** | Critical | §2.1.6 documents JWT issuance from Firebase UID, Google ID, or email resolution but does not require cryptographic proof that the caller owns the supplied identifier. The endpoint must require a verified provider credential, password, or authenticated session before linking identities or issuing tokens; identifier knowledge alone is not authentication. |

...
| 2026-04-27 | `[PENDING REFACTOR]:` Orphan `donationScreens/*` wrappers after DonationsStack trim | `DonationsStack` / deep linking now expose only the eight donation tab categories; `ClothesScreen`, `DreamsScreen`, `CategoryScreen`, and similar files remain in the tree but are no longer registered routes—delete or re-wire if product restores those flows. |

...
- **Operators (מוקדנים)** — call-center brokers who triage high-privacy posts, review needs and capabilities, and manually propose matches between requesters and volunteers/donors (see §2.14). This is a **new persona** requiring the `operator` role (`OperatorAuthGuard` + mobile `useOperatorProtection` / `isOperator` are implemented — full module, DB, and screens still pending — see §10.1)
